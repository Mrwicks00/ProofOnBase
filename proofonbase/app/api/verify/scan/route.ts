...
async function verifyWithGroth16Direct(params: VerificationSubmission) {
  ...
  const baseSepolia = defineChain({
    id: ADDRESSES.baseSepolia.chainId,
    name: "Base Sepolia",
    nativeCurrency: { name: "Ether", symbol: "ETH", decimals: 18 },
    rpcUrls: {
      default: { http: [ADDRESSES.baseSepolia.rpcUrl] },
      public: { http: [ADDRESSES.baseSepolia.rpcUrl] },
    },
    blockExplorers: {
  ...
  console.log(
    ...
    ADDRESSES.baseSepolia.Groth16Verifier
  );
  ...
  const proofValid = await publicClient.readContract({
    address: ADDRESSES.baseSepolia.Groth16Verifier as `0x${string}`,
