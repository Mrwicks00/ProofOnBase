...
export default function ProofOnBaseApp() {
  // RainbowKit hooks
  ...
  const handleRegisterDID = async () => {
    ...
    if (!isCorrectNetwork) {
      toast({
        title: "Wrong Network",
        description: "Please switch to Base Sepolia to register a DID",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleContractVerification = async (verificationData: any) => {
    try {
      ...
      // Use the server-side API that handles AgeGate contract interaction and event parsing
      ...
      if (result.success) {
        // Use the actual isOver18 result from Groth16Verifier
        const isOver18 = result.isOver18 === true;
  ...
  const handleScan = async (text: string) => {
    try {
      ...
      if (result.success) {
        const isOver18 = result.result?.isOver18 === true;
        console.log(
          "PC isOver18 value:",
          isOver18,
          "type:",
          typeof result.result?.isOver18
        );
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const savedTheme = localStorage.getItem("proofonbase-theme") as
        | "light"
        | "dark"
        | null;
      if (savedTheme) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      localStorage.setItem("proofonbase-theme", theme);
          ...
          <TabsContent value="identity" className="space-y-6">
            <Card className="bg-card border-border">
              <CardHeader>
                ...
                <CardDescription>
                  Your unique DID on the Base network
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {!isConnected ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    {!isCorrectNetwork && (
                      <p className="text-sm text-yellow-400 mt-2">
                        Switch to Base Sepolia (Chain ID: 84532) to register a
                        DID
                      </p>
                    )}
                  </div>
          ...
          <TabsContent value="issue" className="space-y-6">
            <Card className="bg-card border-border">
              <CardHeader>
                ...
                <CardDescription>
                  Demo issuer for testing credential issuance on the Base
                  network
                </CardDescription>
              </CardHeader>
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                      <Input
                        id="recipientDID"
                        placeholder="did:proofonbase:example123..."
